...
incendios <- read.csv("n_incendios.csv", header = T, sep = ";")
incendios <- as.data.frame(incendios)
ardida <- read.csv("area_ardida.csv", header = T, sep = ";")
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("Area ardida por Pais", plotOutput("firePlot")),
                  tabPanel("Numero incendios por pais", plotOutput("numberPlot")),
                  tabPanel("Modelo", verbatimTextOutput("summary")),
...
server = function(input, output) {
  ...
  f <- reactive({
    as.formula(paste(ardida[,input$region] ~ incendios[, input$region]))
  })
